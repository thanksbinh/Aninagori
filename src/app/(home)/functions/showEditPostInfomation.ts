...
export async function setAnimeData(
  ...
  if (!!postData?.post_anime_data?.anime_id) {
    animeSearchRef.setLoading(true)
    const promiseArray = [
    ...
    animeSearchRef.setAnimeSearch(postData?.post_anime_data, data?.main_picture?.medium)
    animeSearchRef.setLoading(false)
    const animeData = animeDoc.data()?.animeList.find((anime: any) => {
      return anime.node.id === postData?.post_anime_data?.anime_id
    })
    postAdditionalRef.setData(animeData)
  }
  if (!!postData?.post_anime_data) {
    animeEpisodesRef.setAnimeEpisodes(postData?.post_anime_data?.episodes_seen)
    const status = convertPostFormWatchStatus(postData?.post_anime_data?.watching_progress)
    animeStatusRef.setAnimeStatus(status)
    if (!!postData?.post_anime_data?.total_episodes) {
      animeEpisodesRef.setAnimeTotal(postData?.post_anime_data?.total_episodes)
    }
    if (!!postData?.post_anime_data?.score) {
      animeScoreRef.setAnimeScore(postData?.post_anime_data?.score)
    }
  }
}
...
export function setPostTag(postData: any, animeTagRef: any) {
  ...
  if (!!postData?.post_anime_data?.tag) {
    animeTagRef.setAnimeTag(postData?.post_anime_data?.tag)
  }
  if (!!postData?.tag) {
    animeTagRef.setAnimeTag(postData?.tag)
  }
}
