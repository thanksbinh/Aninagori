...
const SearchBar = () => {
  ...
  const doSearch = async (value: string) => {
    ...
    if (realSearchText.length >= 4) {
      ...
      const usernameQuery = query(usersRef, where("username", "==", realSearchText));
      const querySnapshot = await getDocs(usernameQuery)

      if (querySnapshot.docs)
        setSearchResults(querySnapshot.docs.map(doc => (
          <div
            key={doc.data().username}
            className="flex items-center gap-2 px-4 py-2 text-gray-800 hover:bg-gray-100 cursor-pointer"
            onClick={() => handleResultClick(doc.data().username)}
          >
            <img
              src={doc.data().image ?? '/images/default-profile-pic.png'}
  ...
  return (
    <div ref={ref} className="relative">
      ...
      <button
        type="button"
        className="absolute right-0 top-0 h-full px-4 py-2 text-gray-800 rounded-full hover:bg-gray-200 focus:outline-none focus:ring focus:ring-blue-300"
      >
